...
function remove_money(pl,sc)
	if(mode==0)then
		pl:setScore(pl:getScore("money")-sc)
	elseif (mode ==4) then
...
function removesell(pl,dt)
	if(dt~=nil)then
		cfg.sell[getAllRece()[tonumber(dt[1])+1]] = nil
		save()
...
function removerece(pl,dt)
	if(dt~=nil)then
		cfg.rece[getAllSell()[tonumber(dt[1])+1]] = nil
		save()
